 ...
 */
@Service
public class SubtractOperationServiceIMPL implements OperationServiceInterface {
    @Autowired
    ...
    public OperationResultResponse calculate(Double firstNumber, Double secondNumber) {
        ...
        APIResponse response = calculatorConnectionService.POST(propertyProviderService.getSubServiceCalculationURL(),request);
        return OperationResultResponse.buildFromData(response.getData());
    }

    @Override
    public Double cost() {
        APIResponse response = calculatorConnectionService.GET(propertyProviderService.getSubServiceCostURL());
